...
exFileUploader.attach = function(element, type, fn){
...
exFileUploader.detach = function(element, type, fn){
...
exFileUploader.preventDefault = function(e){
    if (e.preventDefault){
        e.preventDefault();
    } else{
        e.returnValue = false;
 ...
 */
exFileUploader.insertBefore = function(a, b){
    b.parentNode.insertBefore(a, b);
};
exFileUploader.remove = function(element){
 ...
 */
exFileUploader.toElement = (function(){
 ...
 */
exFileUploader.css = function(element, styles){
    if (styles.opacity != null){
        if (typeof element.style.opacity != 'string' && typeof(element.filters) != 'undefined'){
            styles.filter = 'alpha(opacity=' + Math.round(100 * styles.opacity) + ')';
        }
    }
    exFileUploader.extend(element.style, styles);
};
exFileUploader.hasClass = function(element, name){
    var re = new RegExp('(^| )' + name + '( |$)');
    return re.test(element.className);
};
exFileUploader.addClass = function(element, name){
    if (!exFileUploader.hasClass(element, name)){
        element.className += ' ' + name;
    }
};
exFileUploader.removeClass = function(element, name){
    var re = new RegExp('(^| )' + name + '( |$)');
    element.className = element.className.replace(re, ' ').replace(/^\s+|\s+$/g, "");
};
exFileUploader.setText = function(element, text){
    element.innerText = text;
    element.textContent = text;
};

// Selecting elements
exFileUploader.children = function(element){
    var children = [],
...
exFileUploader.getByClass = function(element, className){
    ...
    for (var i = 0; i < len; i++){
        if (exFileUploader.hasClass(candidates[i], className)){
 ...
 */
exFileUploader.obj2url = function(obj, temp, prefixDone){
    var uristrings = [],
        prefix = '&',
    ...
    if (!prefixDone && temp) {
      ...
      uristrings.push(exFileUploader.obj2url(obj));
...
exFileUploader.initialize = function(o){
    ...
    exFileUploader.extend(this._options, {
        ...
        fileTemplate: '<div>' +
                '<div class="ex-upload-spinner"></div>' +
                '<div class="ex-upload-info">' +
                '<span class="ex-upload-file"></span>' +
                '<span class="ex-upload-size"></span>' +
                '<span class="ex-upload-failed-text">Failed</span>' +
                '</div>' +
        ...
        classes: {
            ...
            drop: 'ex-upload-drop-area',
            dropActive: 'ex-upload-drop-area-active',
            list: 'ex-upload-list',

            file: 'ex-upload-file',
            spinner: 'ex-upload-spinner',
            size: 'ex-upload-size',
            cancel: 'ex-upload-cancel',

            // added to list item when upload completes
            // used in css to hide progress spinner
            success: 'ex-upload-success',
            fail: 'ex-upload-fail'
        }
    });
...
exFileUploader.extend(exFileUploader.initialize.prototype, exFileUploader.FileUploaderBasic.prototype);

exFileUploader.extend(exFileUploader.initialize.prototype, {
    /**
    ...
    _setupDragDrop: function(){
        ...
        var dz = new exFileUploader.UploadDropZone({
            element: dropArea,
            onEnter: function(e){
                exFileUploader.addClass(dropArea, self._classes.dropActive);
                e.stopPropagation();
            },
            onLeave: function(e){
                e.stopPropagation();
...
exFileUploader.UploadDropZone.prototype = {
    _disableDropOutside: function(e){
        ...
        if (!exFileUploader.UploadDropZone.dropOutsideDisabled ){
...
exFileUploader.UploadButton.prototype = {
    ...
    _createInput: function(){
        ...
        exFileUploader.attach(input, 'mouseout', function(){
            exFileUploader.removeClass(self._element, self._options.hoverClass);
        });
        exFileUploader.attach(input, 'focus', function(){
            exFileUploader.addClass(self._element, self._options.focusClass);
        });
        exFileUploader.attach(input, 'blur', function(){
            exFileUploader.removeClass(self._element, self._options.focusClass);
        });
 ...
 */
exFileUploader.UploadHandlerAbstract = function(o){
    this._options = {
        debug: false,
        action: '/upload.php',
...
// @inherits exFileUploader.UploadHandlerAbstract
exFileUploader.extend(exFileUploader.UploadHandlerForm.prototype, exFileUploader.UploadHandlerAbstract.prototype);
...
exFileUploader.extend(exFileUploader.UploadHandlerForm.prototype, {
    ...
    _createIframe: function(id){
        ...
        var iframe = exFileUploader.toElement('<iframe src="javascript:false;" name="' + id + '" />');
    ...
    _createForm: function(iframe, params){
        ...
        var form = exFileUploader.toElement('<form method="post" enctype="multipart/form-data"></form>');

        var queryString = exFileUploader.obj2url(params, this._options.action);
 ...
 * @inherits exFileUploader.UploadHandlerAbstract
 */
exFileUploader.UploadHandlerXhr = function(o){
    exFileUploader.UploadHandlerAbstract.apply(this, arguments);
...
// @inherits exFileUploader.UploadHandlerAbstract
exFileUploader.extend(exFileUploader.UploadHandlerXhr.prototype, exFileUploader.UploadHandlerAbstract.prototype)

exFileUploader.extend(exFileUploader.UploadHandlerXhr.prototype, {
    /**
