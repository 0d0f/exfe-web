...
define('routes', function (require, exports, module) {
  ...
  routes.switchPage = function (req, res, next) {
    ...
    var session = req.session
      , isHome = !session.authorization && !session.browsing_authorization && !session.error
      , $body = $(document.body)
      , $navbar = $('#js-navbar');

    $('#js-main > div[role="main"]').empty();

    $body[(isHome ? 'add' : 'remove') + 'Class']('hbg');
    $navbar[(isHome ? 'add' : 'remove') + 'Class']('hide');
      ...
      Api.request('resolveToken'
        ...
        , function (data) {
            if (data) {
              ...
              if (authorization) {
                session.browsing_authorization = auth;
              }
              else {
