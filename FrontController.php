...
//require_once dirname(__FILE__)."/ParamConfig.php";
...
#	}
	...
	#private function checkPara($object, $action) {
	#	global $paras;
	#	$restriction = $paras["${object}s"][$action];
	#	if($restriction["METHOD"] == "GET") {
	#		$requestParas = $_GET;
	#	} else {
	#		$requestParas = $_POST;
	#	}
	#	$requestParas = $_REQUEST;
	#	if(!empty($restriction["MUST"])) {
	#		foreach($restriction["MUST"] as $mustKey) {
	#			if(!array_key_exists($mustKey, $requestParas) && !($mustKey == 'id' && array_key_exists($mustKey, $_GET))) {
	#				echo wrapOutput("", (object)array(), 400, "missing required parameter $mustKey");
	#				exit(0);
	#			}
	#		}
	#	}
